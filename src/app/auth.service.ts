...
import { type Observable, throwError, BehaviorSubject, of, forkJoin } from "rxjs"
...
export class AuthService {
  ...
  authenticate(credentials: { identifier: string; password: string }): Observable<AuthResponse> {
    ...
    return this.http
      .post<any>(`${this.apiUrl}/users/login`, credentials, {
          ...
          if (error.error && error.error.message === "Identifiant invalide") {
            ...
            return this.authenticateTeacher(credentials).pipe(
              ...
              catchError((teacherError) => {
                ...
                const errorMessage =
                  teacherError.error && teacherError.error.message
            ...
            const errorMessage = error.error && error.error.message ? error.error.message : "Erreur d'authentification"
  ...
  private authenticateTeacher(credentials: { identifier: string; password: string }): Observable<any> {
    return this.http
      .post<any>(`${this.apiUrl}/enseignants/login`, credentials, {
        headers: new HttpHeaders({
          "Content-Type": "application/json",
        }),
      })
  ...
  checkCredentials(credentials: { identifier: string; password: string }): Observable<any> {
    return this.http
      .post(`${this.apiUrl}/users/check-credentials`, credentials, {
        headers: new HttpHeaders({
          "Content-Type": "application/json",
        }),
      })
  ...
  private storeStudentInfo(user: any): void {
    ...
    const studentInfo: StudentInfo = {
      ...
      civilStatus: user.civilStatus || undefined,
  ...
  updateStudentProfile(studentData: any): Observable<any> {
    ...
    return this.http
      .put<any>(`${this.apiUrl}/users/${etudiantId}/profile`, studentData, {
        headers: new HttpHeaders({
          "Content-Type": "application/json",
        ...
        tap((updatedUser) => {
          ...
          if (this.studentInfo) {
            this.studentInfo = {
              ...this.studentInfo,
              ...studentData,
              // Conserver les champs qui ne sont pas dans studentData
              _id: this.studentInfo._id,
              role: this.studentInfo.role,
              matricule: this.studentInfo.matricule,
              level: this.studentInfo.level,
              department: this.studentInfo.department,
              specialty: this.studentInfo.specialty,
              filiere: this.studentInfo.filiere,
              group: this.studentInfo.group,
              subGroup: this.studentInfo.subGroup,
              // Conserver les noms mappés
              departmentName: this.studentInfo.departmentName,
              specialtyName: this.studentInfo.specialtyName,
              levelName: this.studentInfo.levelName,
              filiereName: this.studentInfo.filiereName,
              groupName: this.studentInfo.groupName,
            }
  ...
  getSpecialtyInfo(specialtyId: string): Observable<any> {
    ...
    return this.http
      .get<any>(`${this.apiUrl}/specialties/${specialtyId}`, {
        headers: new HttpHeaders({
          "Content-Type": "application/json",
          Authorization: `Bearer ${this.getToken()}`,
        }),
  ...
  getFiliereInfo(filiereId: string): Observable<any> {
    ...
    // Sinon, faire une requête à l'API
    return this.http
      .get<any>(`${this.apiUrl}/filieres/${filiereId}`, {
        headers: new HttpHeaders({
          "Content-Type": "application/json",
          Authorization: `Bearer ${this.getToken()}`,
        }),
  ...
  getGroupInfo(groupId: string): Observable<any> {
    ...
    // Sinon, faire une requête à l'API
    return this.http
      .get<any>(`${this.apiUrl}/groupes/${groupId}`, {
        headers: new HttpHeaders({
          "Content-Type": "application/json",
          Authorization: `Bearer ${this.getToken()}`,
        }),
  ...
  loadAllSpecialties(): Observable<any> {
    return this.http
      .get<any>(`${this.apiUrl}/specialties`, {
        headers: new HttpHeaders({
          "Content-Type": "application/json",
          Authorization: `Bearer ${this.getToken()}`,
        }),
